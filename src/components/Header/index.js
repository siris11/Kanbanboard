...
const groups = ["Status", "Users", "Priority"];
const orders = ["Priority", "Title"];

class Header extends Component {
  state = {
    showFilterContainer: false,
    showGroupByContainer: false,
  ...
  onToggleGroupByContainer = () => {
    this.setState((prevState) => ({
      showGroupByContainer: !prevState.showGroupByContainer,
    }));
  };

  onToggleOrderByContainer = () => {
    this.setState((prevState) => ({
      showOrderByContainer: !prevState.showOrderByContainer,
    }));
  };

  onChangeGroupBy = (event) => {
    const { changeGroupBy } = this.props;
    const updatedGroupBy = event.target.id;
    changeGroupBy(updatedGroupBy.toLowerCase());
    this.setState((prevState) => ({
      showGroupByContainer: !prevState.showGroupByContainer,
  ...
  onChangeOrderBy = (event) => {
    ...
    const updatedOrderBy = event.target.id;
    changeOrderBy(updatedOrderBy.toLowerCase());
    this.setState((prevState) => ({
      showOrderByContainer: !prevState.showOrderByContainer,
    ...
    return (
      <div className="header">
        <div
          ...
          onClick={this.onToggleFilterContainer}
        >
          <svg
        ...
        <ul
          ...
          style={{ display: showFilterContainer ? "flex" : "none" }}
        >
          <li className="filter-type-card">
            <span className="filter-type">Grouping</span>
            <div className="filter-card">
              <div
                className="filter-card-header"
                onClick={this.onToggleGroupByContainer}
              >
                <span>{groupBy}</span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <ul
                ...
                style={{ display: showGroupByContainer ? "block" : "none" }}
              >
                {groups.map((eachItem) => (
                  <li
          ...
          <li className="filter-type-card">
            <span className="filter-type">Ordering</span>
            <div className="filter-card">
              <div
                className="filter-card-header"
                onClick={this.onToggleOrderByContainer}
              >
                <span>{orderBy}</span>
              ...
              <ul
                className="filter-card-list"
                style={{ display: showOrderByContainer ? "block" : "none" }}
              >
                {orders.map((eachItem) => (
                  <li
                    className="filter-name"
