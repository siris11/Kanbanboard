...
const GroupCard = (props) => {
  ...
  const { groupName, tickets } = itemDetails;
    ...
    <div className="group-card">
      <div className="group-card-header">
        <div className="group-card-item">
          {type !== "users" ? groups[type][groupName] : <span>HI</span>}
          <span>
            {type === "priority" ? priorities[groupName] : groupName}
          </span>{" "}
          <span>{count}</span>
        </div>
        <div className="group-card-item">
      ...
      {tickets.map((eachItem) => (
        <TaskCard key={eachItem.id} type={type} itemDetails={eachItem} />
      ))}
    </div>
  );
};
